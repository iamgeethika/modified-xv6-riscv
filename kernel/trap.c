...
void usertrap(void)
{
  ...
  else if ((which_dev = devintr()) != 0)
  {
    int par;
    par = p->alarm_on;
    if (which_dev == 2 && par == 0)
    {
      struct trapframe *trapframe = kalloc();
      const void *k;
      k = p->trapframe;
      memmove(trapframe, k, PGSIZE);
  ...
  {
    int result;
    result = page_fault_handler((void *)r_stval(), p->pagetable);
    if (result == -1)
    {
      p->killed = 1;
    }
...
void usertrapret(void)
{
  ...
  // switches to the user page table, resulttoresult user registers,
...
void kerneltrap()
{
  ...
  // so resulttore trap registers for use by kernelvec.S's sepc instruction.
  w_sepc(sepc);
  w_sstatus(sstatus);
}
